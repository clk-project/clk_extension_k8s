...
platforms = {
    "linux": {
        "x86_64": {
            "kind": f"https://kind.sigs.k8s.io/dl/v{KIND_VERSION}/kind-linux-amd64",
            "helm": f"https://get.helm.sh/helm-v{HELM_VERSION}-linux-amd64.tar.gz",
            "kubectl": f"https://dl.k8s.io/release/v{KUBECTL_VERSION}/bin/linux/amd64/kubectl",
            "tilt": (
                "https://github.com/tilt-dev/tilt/releases/download/"
                f"v{TILT_VERSION}/tilt.{TILT_VERSION}.linux.x86_64.tar.gz"
            ),
            "earthly": f"https://github.com/earthly/earthly/releases/download/v{EARTHLY_VERSION}/earthly-linux-amd64",
        },
        "aarch64": {
            "kind": f"https://kind.sigs.k8s.io/dl/v{KIND_VERSION}/kind-linux-arm64",
            "helm": f"https://get.helm.sh/helm-v{HELM_VERSION}-linux-arm64.tar.gz",
            "kubectl": f"https://dl.k8s.io/release/v{KUBECTL_VERSION}/bin/linux/arm64/kubectl",
            "tilt": (
                "https://github.com/tilt-dev/tilt/releases/download/"
                f"v{TILT_VERSION}/tilt.{TILT_VERSION}.linux.arm64.tar.gz"
            ),
            "earthly": f"https://github.com/earthly/earthly/releases/download/v{EARTHLY_VERSION}/earthly-linux-arm64",
        },
    },
    "darwin": {
        "x86_64": {
            "kind": f"https://kind.sigs.k8s.io/dl/v{KIND_VERSION}/kind-darwin-amd64",
            "helm": f"https://get.helm.sh/helm-v{HELM_VERSION}-darwin-amd64.tar.gz",
            "kubectl": f"https://dl.k8s.io/release/v{KUBECTL_VERSION}/bin/darwin/amd64/kubectl",
            "tilt": (
                "https://github.com/tilt-dev/tilt/releases/download/"
                f"v{TILT_VERSION}/tilt.{TILT_VERSION}.mac.x86_64.tar.gz"
            ),
            "earthly": f"https://github.com/earthly/earthly/releases/download/v{EARTHLY_VERSION}/earthly-darwin-amd64",
        },
        "arm64": {
            "kind": f"https://kind.sigs.k8s.io/dl/v{KIND_VERSION}/kind-darwin-arm64",
            "helm": f"https://get.helm.sh/helm-v{HELM_VERSION}-darwin-arm64.tar.gz",
            "kubectl": f"https://dl.k8s.io/release/v{KUBECTL_VERSION}/bin/darwin/arm64/kubectl",
            "tilt": (
                "https://github.com/tilt-dev/tilt/releases/download/"
                f"v{TILT_VERSION}/tilt.{TILT_VERSION}.mac.arm64.tar.gz"
            ),
            "earthly": f"https://github.com/earthly/earthly/releases/download/v{EARTHLY_VERSION}/earthly-darwin-arm64",
        },
    },
}
...
if urls is None:
    supported_systems = ", ".join(
        [f"{system} ({', '.join(assoc.keys())})" for system, assoc in platforms.items()]
    )
    LOGGER.warning(
        f"We don't support {platform.system().lower()} ({platform.machine()})"
...
class InstallDependency:
    ...
    def need_install(self):
        ...
        if not force and self.found_version != self.needed_version:
            force = True
            LOGGER.info(
                f"Found a different version of {self.name} ({self.found_version})"
        ...
        def wrapper(*args, **kwargs):
            ...
            if self.need_install():
                if urls.get(self.name):
                    LOGGER.info(
                        f"Let me install {self.name} for you at the version {self.needed_version}"
                    )
                    self.install()
                    if self.need_install():
                        LOGGER.warning(
                        ...
                        LOGGER.info(
                            f"{self.name} correctly installed and appears to work"
                        )
                else:
                    LOGGER.warning(
                        f"I don't know how to install {self.name} on your computer."
...
class Kind(InstallDependency):
    ...
    def precondition(self):
        if config.k8s.distribution != "kind":
            LOGGER.status(
                f"I won't try to install kind because you use --distribution={config.k8s.distribution}."
...
class Tilt(InstallDependency):
    ...
    def compute_version(self):
        if self.program_path.exists():
            return re.match(
                "(v[0-9.]+)", check_output([str(Tilt.program_path), "version"])
            ).group(1)
...
def make_earthly_accept_http_connection_from_our_local_registry():
    ...
    if not myregistry.get("http"):
        ...
        config_["global"]["buildkit_additional_config"] = tomli_w.dumps(
            additional_config
        )
...
class Kubectl(InstallDependency):
    ...
    def compute_version(self):
        if self.program_path.exists():
            version_string = safe_check_output(
                [str(Kubectl.program_path), "version", "--client=true"]
            )
            if match := re.match(
                'Client Version: (.+) GitVersion:"(v[0-9.]+)"', version_string
            ):
                ...
                raise click.UsageError(
                    f"Could not identifiy the version of kubectl: {version_string}"
                )
...
class HelmApplication:
    ...
    def _already_installed(self):
        ...
        if releases:
            ...
            if (
                ...
                or "v" + installed_version == self.version
            ):
                if release["status"] != "deployed":
                    LOGGER.warning(
                        f"{self.name} was already installed, but it had the status {release['status']}."
                    )
                    LOGGER.warning(
                        "This may happen when helm reached a timeout but the application was correctly installed."
                    )
                    LOGGER.warning(
                        "Let's try to install it again and see what happens."
                    )
                    return False
                return True
        return False
    ...
    def install(self, force, helm_args):
        if not force and self._already_installed():
            LOGGER.status(
                f"{self.name} already installed in"
        ...
        try:
            ...
            if (
                "content deadline exceeded" in e.content.strip().splitlines()[-1]
                or "timed out waiting" in e.content.strip().splitlines()[-1]
            ):
                LOGGER.warning(
                    "It looks like it was due to a time out,"
...
def guess_context_and_distribution(context, distribution):
    ...
    if context is None and distribution is None:
        LOGGER.debug(
            "Got no hint about the context or the distribution. I will try to guess them"
        ...
        if current_context := config.kubectl.current_context():
            LOGGER.debug(
                "There is a current context! Let's see whether I can make use of it!"
            )
            (
                guessed_context,
            ...
            if guessed_context is not None or guessed_distribution is not None:
                LOGGER.debug(
                    f"Guessed context {guessed_context} and distribution {distribution}"
                )
                ...
                warning = (
                    "I could not infer a suitable distribution"
            ...
            LOGGER.debug(
                f"No current context, falling back on distribution {distribution}"
    ...
    if context is None and distribution is not None:
        ...
        LOGGER.debug(
            f"Given the distribution {distribution}, I inferred the context {context}"
        )
    if context is not None and distribution is None:
        ...
        if context:
            LOGGER.debug(
                f"Given the context {given_context}, I inferred the distribution {distribution}"
            )
        else:
            LOGGER.debug(
                f"Given the context {given_context}, I could not infer any distribution."
...
class K8s:
    ...
    def distribution(self):
        result = guess_context_and_distribution(
            config.kubectl._explicit_context, self._explicit_distribution
        )[1]
        if result is None:
            raise click.UsageError(
                "I could not infer a suitable distribution. Try providing one explicitly."
            )
        return result
...
class KubeCtl:
    ...
    def context(self):
        result = guess_context_and_distribution(
            self._explicit_context, config.k8s._explicit_distribution
        )[0]
        if result is None:
            raise click.UsageError(
                "I could not infer a suitable context. Try providing one explicitly."
            )
        return result
    ...
    def get(self, kind, name=None, namespace="default", internal=False):
        ...
        return [
            elem
            for elem in json.loads(
                config.kubectl.output(
                    ["get", kind, "--namespace", namespace, "--output", "json"]
                )
            )["items"]
...
def doctor():
    ...
    if kube_context := config.kubectl.current_context():
        LOGGER.info(
            f"Trying to play with the kubernetes available with the context {kube_context}"
        )
        LOGGER.info(
            "I will check that kubectl build works correctly."
...
""")
            try:
                ...
                warnings += 1
                LOGGER.warning(
                    "I could not run kubectl build."
                    " This is not a blocker but you won't be able to take advantage of fast builds."
                )
    else:
        LOGGER.warning(
            "It looks like there is no kubernetes available so far."
            " Run this command again after having started your stack so that I can make more tests."
        )
    if warnings:
        LOGGER.info(
            f"We found {warnings} reason{'s' if warnings else ''} for you to worry about the stack."
        )
    else:
        LOGGER.info(
...
@option(
    "--username",
    help=(
        "The username of the provider registry"
...
@option(
    "--password",
    help=(
        "The password of the provider registry"
...
@flag(
    "--force",
    help="Overwrite the existing secret, default for aws that has very short lived token",
)
@option(
...
@option(
    "--k8s-login/--no-k8s-login",
    default=True,
    help="Also add the secret into the local cluster",
)
def registry_login(
    registry_provider,
    username,
    ...
    if not (username and password):
        if username or password:
            LOGGER.warning(
                "I need to be given both username and password to use them."
                " Falling back on trying using the keyring."
            )
        if res := get_keyring().get_password(
            "clk", f"{registry_provider}-registry-auth-user"
        ):
            username = res
        else:
            LOGGER.warning("Could not find the username from your password manager")
        if res := get_keyring().get_password(
            "clk", f"{registry_provider}-registry-auth-password"
        ):
            password = res
        else:
...
def wait_ready():
    ...
    while not all(
        ...
        if tries > threshold:
            msg += (
                " It's been a long time now, something may be wrong."
    ...
    if tries > threshold:
        LOGGER.warning(
            "The cluster has finally begun correctly."
...
@flag(
    "--use-public-dns",
    help="Don't rely on the host dns fallback resolving (only implemented in kind)",
)
def create_cluster(recreate, nodes, api_server_address, calico_version, use_public_dns):
    ...
    if config.dry_run:
        LOGGER.info(
            f"(dry-run) create a {config.k8s.distribution} cluster."
    ...
    if config.k8s.distribution == "kind":
        ...
        if name in silent_check_output([Kind.program_path, "get", "clusters"]).split(
            ...
            if recreate:
                ...
                LOGGER.status(
                    f"A cluster with the name {name} already exists. Nothing to do."
                )
                return
    else:
    ...
    if not is_port_available(80):
        raise click.ClickException(
            "Port 80 is already in use by another process. Please stop this process and retry."
        )
    if not is_port_available(443):
        raise click.ClickException(
            "Port 443 is already in use by another process. Please stop this process and retry."
        )
...
@cert_manager.command(
    flowdepends=["k8s.install-ingress-controller"], handle_dry_run=True
)
@option(
    "--version",
...
@option(
    "--ca-key-path",
    type=Path,
    help="The key to use instead of generating one dynamically",
)
@option(
    "--ca-crt-path",
    type=Path,
...
def generate_certificate_authority(ca_key_path, ca_crt_path):
    ...
    if ca_crt_path and not ca_key_path:
        raise click.UsageError(
            "--ca-key-path must be used when --ca-crt-path is provided"
        )
    if config.dry_run:
        LOGGER.info(
            "(dry-run) generating a certificate authority."
...
@option(
    ...
    default="browsers",
    help=(
        "Install the certificate for the given client."
...
def install_local_certificate(client, secret_name, namespace, type, output_file):
    ...
    if os_name == "linux":
        ...
        if update_ca_certificates is None and client == "ca-certificates":
            LOGGER.error(
                "You have to install ca-certificates to use this command."
        ...
        raise NotImplementedError(
            f"Operating system not supported {os_name}, supported systems are: darwin, linux"
        )
    ...
    with temporary_file() as f:
        ...
        if os_name == "darwin":
            LOGGER.info(
                "I need your sudo password to use security to install the local certificate."
            ...
            if client in ("ca-certificates", "all"):
                call(
                    ...
                    + ["mkdir", "-p", "/usr/local/share/ca-certificates/mine"]
                )
                call(
                    (["sudo"] if os.getuid() != 0 else [])
        ...
        if not did_something:
            raise NotImplementedError(
                f"Sounds like we did not actually deal with the client {client}"
            )
...
@option(
    "--other-domain",
    multiple=True,
    help="Some other domain to associate with this IP address",
)
    ...
    if config.k8s.distribution == "kind":
        ...
        if reset:
            new_value = "\n".join(
                [
                    line
                    for line in coredns_conf["data"]["Corefile"].splitlines()
                    if not line.endswith(watermark)
                ]
            )
        ...
        hosts {
            ...
            coredns_conf["data"]["Corefile"] = (
                coredns_conf["data"]["Corefile"][0:last_bracket_index] + data + "\n}\n"
            )
            update = True
...
def minimal():
    ...
    if not config.dry_run:
        LOGGER.status(
            "Everything worked well. Now enjoy your new (minimal) cluster ready to go!"
        )
...
class Chart:
    ...
    def __init__(self, location):
        ...
        if not self.index_path.exists():
            raise click.UsageError(
                f"No file Chart.yaml in the directory {self.location}."
        ...
        self.dependencies_fullnames = [
            self.compute_name(dep) for dep in self.dependencies
        ]
    ...
    def sanity_check_dependencies(self):
        ...
        non_unique_deps = [
            f"{key[0]}-{key[1]} as {key[2]}" for key, value in deps.items() if value > 1
        ]
        if non_unique_deps:
            raise ChartDuplicatedDependencies(
                f"{', '.join(non_unique_deps)} are non unique in {self.location}"
            )
    ...
    def match_to_dependencies(self, name):
        ...
        return [
            dependency
            for dependency in self.dependencies_fullnames
        ...
        with tempdir() as d, cd(d):
            ...
            if dest.exists():
                if (
                    ...
                    LOGGER.status(
                        f"Not overwriting {dest} already with the appropriate content"
                    )
                else:
                    rm(dest)
                    move(src, dest)
    ...
    def get_dependencies_with_helm(self, deps_to_update):
        ...
        with tempdir() as d, open(f"{d}/Chart.yaml", "w") as f:
            ...
            LOGGER.status(
                "## The following is some helm logs, don't pay much attention to its gibberish"
            )
        ...
        LOGGER.status(
            f"Downloaded {', '.join([self.compute_name(dep) for dep in deps_to_update])} for {self.name}"
        )
    ...
    def find_one_source(cls, dependency, subchart_sources):
        ...
        match = [
            chart
            for chart in subchart_sources
            if cls.compute_name(dependency).startswith(chart.name)
        ]
        if len(match) > 1:
            raise NotImplementedError()
        if not match:
            loose_matches = [
                chart
                for chart in subchart_sources
                if cls.compute_short_name(dependency).startswith(
                    cls.compute_short_name(chart.index)
                )
            ]
        ...
        if cls.compute_name(dependency) != match.name:
            LOGGER.warning(
                f"I guessed that the provided package {match.name} (available at {match.location})"
    ...
    def update_dependencies(self, subchart_sources, force=False):
        ...
        for dependency in self.dependencies:
            ...
            if src is not None:
                LOGGER.status(
                    f"Using {src.name} (from {src.location}) to fulfill dependency {dependency_name}"
                )
                ...
                LOGGER.status(
                    f"I will unconditionally download {dependency_name} as a dependency of {self.name}"
                    " (because of --force)"
                )
                ...
                LOGGER.status(
                    f"{dependency_name} is already an up to date dependency of {self.name}"
                )
        ...
        if to_fetch_with_helm_unique:
            generated_dependencies = self.get_dependencies_with_helm(
                to_fetch_with_helm_unique
            )
            ...
            with tempdir() as d:
                for dependency_to_resolve in generated_dependencies | to_resolve:
                    dependency_chart_location = (
                        self.subcharts_dir / dependency_to_resolve
                    )
                    temp_dependency_location = (
                        Path(d) / Path(dependency_to_resolve).name
                    )
                    ...
                    updated_subcharts = dependency_chart.resolve_subcharts(
                        subchart_sources=subchart_sources
                    )
                    if updated_subcharts:
                        LOGGER.status(
                            f"In {self.location}, substituting {dependency_chart.name} by the resolved one"
                        )
    ...
    def resolve_subcharts(self, subchart_sources):
        ...
        for subchart_dir in self.subcharts_dir.iterdir():
            if subchart_dir.is_dir():
                ...
                if src is not None:
                    LOGGER.status(
                        f"Substituting {subchart.location} by the source {src.name} from {src.location}"
                    )
                    ...
                    updated = (
                        subchart.resolve_subcharts(subchart_sources=subchart_sources)
                        or updated
                    )
        return updated
...
@option(
    ...
    type=Chart,
    help=(
        "Directory of a helm package that can be used to override the dependency fetching mechanism"
    ),
)
@option(
    "--remove/--no-remove",
...
@flag(
    "--uncompress",
    help=("Also leave out an uncompressed version. Ideal for grepping into them."),
)
@argument("chart", default=".", type=Chart, required=False, help="Helm chart path")
def dependency_update(
    chart, force, touch, experimental_oci, subchart_sources, remove, uncompress
):
...
def setup_credentials():
    ...
    """
    LOGGER.debug(
        "No credentials added in the cluster."
...
@option(
    "--set-key",
    help=(
        "Make the command output the given key with the given value."
...
def features(fields, format, keys, set_key):
    ...
    if config.k8s.distribution == "kind":
        reg_name = f"{config.k8s.distribution}-registry"
        _features[config.k8s.distribution]["local_registry"] = (
            reg_name in check_output(split("docker ps --format {{.Names}}")).split()
        )
...
def __install(strict, force):
    ...
    if config.dry_run:
        LOGGER.info(
            "(dry-run) run kubectl apply to install some network policies. "
...
def _run(open, use_context, tilt_arg, tiltfile_args, label, namespace, down):
    ...
    if not (root / tiltfile_name).exists():
        raise click.UsageError(
            f"I looked for a file called {tiltfile_name} in this"
...
class NamespaceNameType(DynamicChoice):
    def choices(self):
        return [
            get_resource_name(item)
            for item in config.kubectl.json(["get", "namespaces"], internal=True)[
                "items"
            ]
        ]
...
def write_kubectl_config(output):
    ...
    item = [
        item
        for item in config.kubectl.json(["-n", config.namespace, "get", "secret"])[
            "items"
        ]
        if item["metadata"]
        .get("annotations", {})
    ...
    contexts = [
        context
        for context in current_conf["contexts"]
        if context["name"] == config.kubectl.context
    ]
    contexts[0]["context"]["user"] = config.sa_name
    assert len(contexts) == 1
    clusters = [
        cluster
        for cluster in current_conf["clusters"]
        if cluster["name"] == contexts[0]["context"]["cluster"]
    ]
    new_conf = {
        "kind": "Config",
        "apiVersion": "v1",
    ...
    persistentVolumeClaim:
      claimName: {pvc_name}"""
        ...
        try:
            ...
            config.kubectl.call(
                ["exec", "-i", "-t", podname, "--"] + command, silent=False
            )
        finally:
...
def resources():
    ...
    config.tilt.label = [
        label for label in config.tilt.label if label not in config.tilt.but
    ]
...
def able_command(direction):
    ...
    resources.command(
        name=direction, help=f"{direction.capitalize()} the selected resources"
    )(able)
...
@k8s.command()
@argument(
    "context", help="Use that context", type=K8sContextType(), expose_class=K8sgoConfig
)
@argument(
    "namespace",
